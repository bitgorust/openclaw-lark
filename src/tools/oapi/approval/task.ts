...
export function buildApprovalTaskRequest(
  params: Extract<FeishuApprovalTaskParams, { action: 'approve' | 'reject' | 'transfer' }>,
  ...
  return {
    ...
    path: `/open-apis/approval/v4/tasks/${params.action}`,
  };
}
...
export function registerFeishuApprovalTaskTool(api: OpenClawPluginApi): void {
  ...
  registerTool(
    api,
    {
      ...
      description:
        '【以应用身份】飞书审批任务工具。用于同意、拒绝、转交或退回审批任务。Actions: approve, reject, transfer, rollback。默认把当前消息发送者 open_id 作为操作人 user_id。',
