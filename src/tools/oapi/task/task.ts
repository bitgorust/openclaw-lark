...
export function registerFeishuTaskTaskTool(api: OpenClawPluginApi): void {
  ...
  registerTool(
    api,
    {
      ...
      description:
        "【以用户或应用身份】飞书任务管理工具。用于创建、查询、更新任务，并支持 add_members（添加任务成员）。list 返回当前身份可见的任务列表。时间参数使用 ISO 8601 / RFC 3339 格式（包含时区），例如 '2024-01-01T00:00:00+08:00'。支持通过 auth_type 参数切换用户(user)或应用(tenant)身份。",
